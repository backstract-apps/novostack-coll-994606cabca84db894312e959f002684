...
@router.get('/b_user/')
async def get_b_user( id: str, user_name: str , db: Session = Depends(get_db)):
    return await service.get_b_user(db , id, user_name)
...
@router.post('/b_user/')
async def post_b_user( id: str, first_name: str, last_name: str, age: int, pincode: int , db: Session = Depends(get_db)):
